...
Date: 2026-09-17 · Baseline commit: `3bc840d`
...
| 3 | RAG not reading the database | Embedding-space mismatch between ingested vectors and the configured provider; vector-only retrieval missed lexical matches; **a pronoun-only follow-up retrieved nothing at all** | Hybrid retrieval (vector + lexical) with embedding-space metadata; short follow-ups carry the previous question's topic into the query | `test_knowledge_search.py`, `test_rag_context.py`, `test_lsd_knowledge_retrieval.py` | LOCAL ONLY — the **client must re-index** with the configured provider; real PDF retrieval CLIENT REQUIRED |
...
| 7 | Behaviour does not match the helpline reference | Prompt was a generic business assistant: no helpline role, no guardrails, no escalation, no source attribution | `AI_PERSONA=lsd_helpline` persona (short farmer-friendly turns, no diagnosis, no medicine names, no prices/schemes, escalation, NDDB attribution) + verbatim district directory the model may not add to | `test_helpline_persona.py` | LOCAL ONLY — **district centre list still to be supplied**; spoken wording review CLIENT REQUIRED |
...
now, so the full suite result is real rather than partial. The 37 tests added
since the last report cover the helpline persona, the referral directory and
Lumpy Skin Disease retrieval.

## Must be done on the client machine

1. **Supply the district → diagnostic centre list** and set
   `REFERRAL_DIRECTORY_PATH` (see `docs/HELPLINE_BEHAVIOUR.md`). Until then
   the agent names no centre at all, which is the safe state, not the
   intended one.
...
3. `pip install -r requirements.txt` (numpy is required), `alembic upgrade
...
5. Real GSM calls: IVR digits, department routing with staff mobiles,
...
6. Click through the admin panel in a browser.
7. One 30-minute call for category L.
